...
export const Hero = () => {
    ...
    return (
          <section className="relative min-h-screen flex items-center px-[6%] pt-28 pb-20 overflow-hidden bg-navy" id="home">
                <div className="absolute inset-0 circuit-bg pointer-events-none"></div>div>
                <div className="absolute -top-24 -left-24 w-[500px] h-[500px] bg-brand-red/5 rounded-full blur-[100px] pointer-events-none"></div>div>
                <div className="absolute -bottom-24 -right-0 w-[600px] h-[600px] bg-navy-lighter/80 rounded-full blur-[100px] pointer-events-none"></div>div>
                <div className="absolute top-0 right-0 w-[45%] h-full bg-gradient-to-br from-transparent via-transparent to-brand-red/5 pointer-events-none"></div>div>
                <div className="absolute top-0 right-0 w-0.5 h-full bg-gradient-to-b from-brand-red to-transparent"></div>div>
                ...
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12 items-center w-full">
                        <motion.div
                                  ...
                                  <div className="flex items-center gap-2.5 mb-6">
                                              <div className="w-2 h-2 bg-brand-red rounded-full animate-blink-dot shrink-0"></div>div>
                                  ...
                                  <h1 className="font-display text-6xl md:text-7xl lg:text-8xl font-black leading-[1.1] uppercase tracking-tight mb-7">
                                              <span className="block">{t('Powering', 'Kuleta Nguvu')}</span>span>
                                              <span className="text-brand-red block">{t('Your World', 'Ulimwengu Wako')}</span>span>
                                  </h1>h1>
                                  ...
                                  <div className="grid grid-cols-2 md:grid-cols-4 border-t border-white/10">
                                    {stats.map((stat, i) => (
                                                  <div
                                                                    key={i}
                                                                    className={`py-6 pr-6 md:pr-10 md:py-8 border-white/10 ${i % 2 === 0 ? 'border-r' : ''} ${i < 2 ? 'border-b md:border-b-0' : ''} md:border-r md:last:border-r-0 `}
                                                                  >
                                                                  <div className="font-display text-4xl font-black text-brand-red leading-none">{stat.n}</div>div>
                                                                  <div className="text-[10px] text-brand-gray tracking-widest uppercase mt-1">{t(stat.en, stat.sw)}</div>div>
                                                  </div>div>
                                                ))}
                                  </div>div>
                        ...
                        <motion.div
                                  ...
                                  >
                                  <img src={industrialImg} alt="Industrial Project" className="w-full h-full object-cover" />
                                  <div className="absolute -inset-6 border border-brand-red/10 rounded-[40px] -z-10 animate-pulse"></div>div>
                        </motion.div>motion.div>
                </div>div>
          </section>section>
        );
